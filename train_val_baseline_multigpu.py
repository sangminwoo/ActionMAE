...
from lib.utils.gradient_modulation import calculate_coefficient, update_model_with_OGM_GE
...
def train_setup(logger):
    ...
    args.lr = args.lr*float(args.bs*args.world_size)/32
...
def train_epoch(model, dataloader, optimizer, device, epoch_i):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        loss_mask_dict, loss_label_dict, preds_dict, feats = model(targets, **model_inputs)
        ...
        ###########################################################################################
        # Gradient Modulation
        if args.use_gradient_modulation and len(args.modality.split('_')) > 1:
            coeffs = calculate_coefficient(targets, feats, alpha=args.modulation_ratio)
            update_model_with_OGM_GE(model, coeffs, generalization_enhancement=False)
        ###########################################################################################
...
def val_epoch(model, dataloader, optimizer, device, epoch_i):
    ...
    weight_dict = {
        'loss_mask': args.set_loss_mask,
        'loss_label': args.set_loss_label
    }
    ...
    for idx, batch in enumerate(dataloader):
        ...
        # for k, (acc1, acc5, num_items) in zip(ACTION_CLASSES, per_cls_acc):
        #     per_cls_acc_meters[f'"{k}" acc@1'].update(
        #         val=float(acc1),
        #         n=num_items,
        #     )
        #     per_cls_acc_meters[f'"{k}" acc@5'].update(
        #         val=float(acc5),
        #         n=num_items,
        #     )
...
def train_val(logger, run=None):
    ...
    if args.local_rank == 0:
        ...
        # logger.info([name for name, params in model.named_parameters()])
            ...
            if acc > best_acc:
                ...
                torch.save(
                    ...
                    os.path.join(
                        ...
                        f'{args.fusion}_{args.set_loss_mask}_{args.set_loss_label}.ckpt'
                    )
                )
            else:
                early_stop_count += 1
